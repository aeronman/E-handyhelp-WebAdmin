...
const ViewFeedbacks = () => {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:5000/api/feedback') // Replace with your backend URL
            .then((response) => {
            ...
            {selectedFeedback && (
                <Modal show={showModal} onHide={handleCloseModal}>
                    ...
                    <Modal.Body>
                        ...
                        {sentimentResult && (
                            <div>
                                <p><strong>Sentiment Score:</strong> {sentimentResult.score}</p>
                                <p><strong>Sentiment:</strong> {sentimentResult.score > 0 ? 'Positive' : sentimentResult.score < 0 ? 'Negative' : 'Neutral'}</p>
                            </div>
                        )}
                    </Modal.Body>
                    <Modal.Footer>
